...
**Tumor vs Normal analysis for Head and Neck cancer dataset - GSE65858 under different conditions:** 
...
plot(mds_HN_tissue_6791[,1],mds_HN_tissue_6791[,2],bg=as.numeric(cancer_tissue_status),pch=21,xlab="First dimension",ylab="Second dimension",cex=2,main="MultiDimension Scaling PLot: cancer & HPV status")
legend("topleft",levels(cancer_tissue_status),col=seq(along=levels(cancer_tissue_status)),pch=15,cex=0.6)
table(cancer_tissue_status)
identify(mds_HN_tissue_6791)
sample_info_HN_tissue[c(1,3,4,5,22,23,28,34,38,50,53,54,55,56),c(2,4,6)]
...
# look at dendrogram and check if this 4th sample is clustered with other cancer samples
# dendrogram to see which samples are close or far
hierarchial_cluster_tumorHPVpositive_normal <- hclust(dist_matrix_tumorHPVpositive_normal)
plot(hierarchial_cluster_tumorHPVpositive_normal,cex=0.8,main="Hierarchical clustering of samples",label=cancer_status_tumorHPVpositive_normal)
plot(hierarchial_cluster_tumorHPVpositive_normal,cex=0.8,main="Hierarchical clustering of samples")
sample_info_tumorHPVpositive_normal[c("GSM155718","GSM155717","GSM155711","GSM155677"),c(2,4,6)]
```
...
# look at dendrogram and use both HPV status, cancer status and tissue as labels
hierarchial_cluster_tumorHPVnegative_normal <- hclust(dist_matrix_tumorHPVnegative_normal)
plot(hierarchial_cluster_tumorHPVnegative_normal,cex=0.8,main="Hierarchical clustering of samples",label=cancer_status_tumorHPVnegative_normal)
plot(hierarchial_cluster_tumorHPVnegative_normal,cex=0.8,main="Hierarchical clustering of samples")

#### look at the tissues
sample_info_tumorHPVnegative_normal[c("GSM155717","GSM155712","GSM155674"),c(2,4,6)]
```
